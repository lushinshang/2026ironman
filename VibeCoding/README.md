...
> 📦 **5 天一循環的主題分類**：Day1-5＝個人寫作發布流程；Day6-10＝甲方招標文件版本管控（多次公告→分析差異避免重複評估規劃→AI 整理前去識別化→離線還原）。原本 Day2「PDF 轉 TXT/MD」、Day3「文字差異比對器」、Day4「文件去識別化工具」因主題重新分類，已分別搬移至現在的 Day6、Day7、Day9（`git mv`，內容與開發紀錄不變，各檔案已加註原編號）；空出來的 Day2～4 是新主題。Day2 原定「中英文排版工具」，經場景腦力激盪與需求強度檢視後換題為「贅詞與 AI 味檢查器」，已完成開發；Day3 原定「台灣用語轉換器」，併入 100 題清單 #11 繁簡轉換器擴大為「繁簡與在地化轉換器」，已完成開發；Day4 尚待開發。
...
| 11 | ✅ 繁簡轉換器（已併入 [Day3](Day3/index.html)） | 兩岸文件交換需要轉換 ➡️ 支援繁簡與常見詞彙轉換 ➡️ 貼上後複製到正式文件 |
| 12 | ✅ 台灣用語轉換器（已併入 [Day3](Day3/index.html)） | 文件含有不符合台灣習慣的用語 ➡️ 依可編輯詞庫批次替換 ➡️ 公開發布前快速檢查 |
