...
> 依各 `DayN/` 資料夾的實際檔案與 README checklist 狀態核對，不是人工估算。之後每完成一天，重新核對對應資料夾再更新這張表。

**總覽**：6 / 30 天完成（20%）

> 📦 **5 天一循環的主題分類**：Day1-5＝個人寫作發布流程；Day6-10＝甲方招標文件版本管控（多次公告→分析差異避免重複評估規劃→AI 整理前去識別化→離線還原）。原本 Day2「PDF 轉 TXT/MD」、Day3「文字差異比對器」、Day4「文件去識別化工具」因主題重新分類，已分別搬移至現在的 Day6、Day7、Day9（`git mv`，內容與開發紀錄不變，各檔案已加註原編號）；空出來的 Day2～4 是新主題。Day2 原定「中英文排版工具」，經場景腦力激盪與需求強度檢視後換題為「贅詞與 AI 味檢查器」，已完成開發；Day3、Day4 尚待開發。

| Day | 主題 | 開發 | 撰文 | 連結 |
|:---:|---|:---:|:---:|---|
| 1 | 字數與段落統計器（100 題清單 #9） | ✅ 已完成，含離線下載功能 | ✅ 草稿與總編潤飾完成 | [工具](Day1/index.html) ・ [PRD](Day1/PRD.md) ・ [文章](Day1/article_draft.md) |
| 2 | 贅詞與 AI 味檢查器（原定「中英文排版工具」100 題清單 #10，經腦力激盪與需求強度檢視後換題；個人寫作發布流程第 2 步：定稿前檢查） | ✅ 已完成，fresh agent 獨立驗收 11 條驗收條件全數通過，另加測 3 項邊界情況 | ✅ 草稿完成，待總編潤飾 | [工具](Day2/index.html) ・ [PRD](Day2/PRD.md) ・ [文章](Day2/article_draft.md) |
| 3 | ⬜ 台灣用語轉換器（100 題清單 #12，個人寫作發布流程第 3 步：在地化用語校正） | ⬜ 待開發 | ⬜ 待開發 | — |
...
| 7 | 文件版本差異比對（原 Day3，100 題清單 #3，逐句預設比對＋摺疊未變動區塊；命名維持通用「舊版/新版」，可重複用於招標公告以外的版本比對場景） | ✅ 已完成，含 6 輪真實文件追加修正（fresh agent／codex exec 對抗式驗收） | ✅ 草稿與總編潤飾完成，GitHub Pages 連結已上線（curl -sI 200） | [工具](Day7/index.html) ・ [PRD](Day7/PRD.md) ・ [資安檢測](Day7/SECURITY_REVIEW.md) ・ [文章](Day7/article_draft.md) |
| 8 | 異動重點摘要產生器（新，吃 Day7 差異報告 MD，逐條可勾選，組成可編輯可下載的條列摘要，避免重複評估規劃；含四項雜訊過濾規則，經三輪真實文件迭代修正） | ✅ 已完成，fresh agent 獨立驗收 12＋11＋8＋8 條驗收條件全數通過 | ✅ 草稿與總編潤飾完成，GitHub Pages 連結已上線（curl -sI 200） | [工具](Day8/index.html) ・ [PRD](Day8/PRD.md) ・ [文章](Day8/article_draft.md) |
| 9 | 可逆式去識別化工具（原 Day4，延伸為可逆版：個資部分遮蔽改為純 token 代碼如 `[姓名_01]`，同類型不同人分配專屬序號避免碰撞，機敏字庫代換維持原樣，待新增下載代碼對照表 CSV/JSON） | ✅ 原版已完成三輪擴充，fresh agent 獨立驗收 9＋11＋6 條驗收條件全數通過；⬜ 可逆化＋對照表延伸尚未開發 | ✅ 原版草稿與總編潤飾完成，fresh agent 獨立驗收 5 條條件全數通過 | [工具](Day9/index.html) ・ [PRD](Day9/PRD.md) ・ [資安檢測](Day9/SECURITY_REVIEW.md) ・ [文章](Day9/article_draft.md) |
| 10 | ⬜ 離線代入還原工具（新，貼上 AI 整理後仍含代碼的文字/txt/md ＋ 上傳對照表，離線做代碼→原文全域取代還原，與 Day9 首尾呼應） | ⬜ 待開發 | ⬜ 待開發 | — |

<details>
<summary>Day 11～30（其餘題目待從下方 100 題清單排定）</summary>
...
| 59 | Windows 轉 WSL 路徑工具 | Windows 路徑交給 WSL 指令時常打錯 ➡️ `C:\Users\...` 轉成 `/mnt/c/Users/...` ➡️ 交給 Windows 使用者測試 |
